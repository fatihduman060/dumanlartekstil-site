...
<div class="pos-shell" data-pos-root data-history-audit="<?php echo $canDeleteSales ? '1' : '0'; ?>" data-today="<?php echo e(date('Y-m-d')); ?>" data-api="barkod-satis-api.php" data-csrf="<?php echo e(csrf_token()); ?>">
  ...
  <section class="pos-main panel-card">
    <div class="pos-title-row">
      <div><span class="pos-kicker">MAĞAZA KASASI</span><div class="pos-heading-line"><h2>Yeni Satış</h2><a class="btn btn-secondary pos-new-sale" href="barkod-satis.php" target="_blank" rel="noopener" aria-label="Yeni satışı yeni sekmede aç">+ Yeni Satış</a><button type="button" class="btn btn-secondary pos-price-check-open" data-price-check-open>₺ Fiyat Bak</button><a class="btn btn-secondary pos-product-entry-button" href="barkod-urunler.php">+ Yeni Ürün Ekle</a><a class="btn btn-secondary pos-product-stock-button" href="barkod-urunler.php#stokta-urunler">Stoktaki Ürünler</a></div><p>Barkodu okut veya ürün adından herhangi bir kelimeyi yaz. Son ürünü artırmak için örneğin +3 yazman yeterli.</p></div>
  ...
  <div class="pos-payment-modal" data-pos-payment-modal hidden role="dialog" aria-modal="true" aria-labelledby="posPaymentTitle">
    <div class="pos-payment-dialog">
      ...
      <fieldset class="pos-payments"><legend>Ödeme şekli</legend>
        ...
        <label><input type="radio" name="pos_payment" value="mixed" /><span>💵 + 💳 Nakit + Kredi Kartı</span></label>
        <label><input type="radio" name="pos_payment" value="credit" /><span>🧾 Veresiye</span></label>
      </fieldset>
      <div class="pos-split-payment" data-pos-split-wrap hidden>
        <strong>Bölünmüş ödeme</strong>
        <small>Nakit tutarını yazın; kalan tutar kredi kartına otomatik aktarılır.</small>
        <label><span>Nakit</span><input type="number" min="0.01" step="0.01" inputmode="decimal" placeholder="0,00" data-pos-split-cash /></label>
        <label><span>Kredi Kartı</span><input type="number" step="0.01" readonly tabindex="-1" data-pos-split-card /></label>
        <small>Satış toplamı: <strong data-pos-split-total>0,00 TL</strong></small>
      </div>
  ...
  <section class="panel-card pos-products-panel pos-products-legacy" aria-hidden="true">
    ...
    <form class="pos-product-form" data-product-form>
      ...
      <div class="wide pos-extra-barcodes"><span class="pos-field-label">Ek barkodlar</span><div class="pos-extra-barcode-add"><input type="text" autocomplete="off" placeholder="Diğer barkodu okutun" data-extra-barcode-input data-barcode-input /><button class="btn btn-secondary" type="button" data-extra-barcode-add>Ekle</button></div><input type="hidden" name="extra_barcodes" value="" /><div class="pos-extra-barcode-list" data-extra-barcode-list><small>Henüz ek barkod yok.</small></div><small>Eklediğiniz barkodların tamamı aynı ürünü, fiyatı ve stoğu kullanır.</small></div>
    ...
    <div class="pos-product-manager" data-product-manager hidden>
      <div class="pos-product-manager-toolbar"><label><span>Ürünlerde ara</span><input type="search" autocomplete="off" placeholder="Ürün adı, barkod veya artikel" data-product-list-search /></label><button type="button" class="btn btn-primary" data-product-bulk-save>Tüm Değişiklikleri Kaydet</button></div>
      <p class="pos-product-manager-status" data-product-manager-status></p>
      <div class="pos-product-table-wrap"><table class="pos-product-table"><thead><tr><th>Ürün</th><th>Barkod</th><th>Satış fiyatı</th><th>Stok adedi</th><th>İşlem</th></tr></thead><tbody>
      <?php foreach ($products as $p): $variant = trim((string)($p['variant_name'] ?? '')); $searchText = mb_strtolower($p['name'] . ' ' . $variant . ' ' . implode(' ', $p['barcodes'] ?? [$p['barcode']]), 'UTF-8'); ?>
        <tr data-bulk-product="<?php echo e($p['id']); ?>" data-product-search="<?php echo e($searchText); ?>"><td data-label="Ürün"><strong><?php echo e($p['name'] . ($variant !== '' ? ' - ' . $variant : '')); ?></strong></td><td data-label="Barkod"><span><?php echo e($p['barcode']); ?></span><?php if ((int)($p['barcode_count'] ?? 1) > 1): ?><small><?php echo e((int)$p['barcode_count']); ?> barkod</small><?php endif; ?></td><td data-label="Satış fiyatı"><input type="number" min="0.01" step="0.01" value="<?php echo e(number_format((float)$p['sale_price'], 2, '.', '')); ?>" data-bulk-price /></td><td data-label="Stok adedi"><input type="number" step="1" value="<?php echo e(number_format((float)$p['stock_quantity'], 0, '.', '')); ?>" data-bulk-stock /></td><td data-label="İşlem"><button type="button" class="btn btn-secondary pos-product-edit-button" data-product-edit='<?php echo e(json_encode($p, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)); ?>'>Düzenle</button></td></tr>
      <?php endforeach; ?>
      </tbody></table></div>
...
<script>
(function(){var slot=document.querySelector('.pos-checkout-slot');if(!slot||!window.matchMedia('(min-width:981px)').matches)return;var lock=function(){slot.classList.remove('is-locked');slot.style.removeProperty('--pos-checkout-sticky-top');requestAnimationFrame(function(){var naturalTop=Math.round(slot.getBoundingClientRect().top+window.scrollY);if(naturalTop<0)naturalTop=0;slot.style.setProperty('--pos-checkout-sticky-top',naturalTop+'px');slot.classList.add('is-locked');});};if(document.readyState==='complete')lock();else window.addEventListener('load',lock,{once:true});})();
</script>
...
<script src="assets/barkod-read-json.js?v=1"></script>
<script src="assets/barkod-canli-sepet.js?v=3"></script>
<script src="assets/barkod-satis.js?v=35"></script>
<script src="assets/barkod-canli-arama.js?v=9"></script>
<script src="assets/barkod-veresiye-yeni-kisi.js?v=1"></script>
<script src="assets/barkod-cuma-hizli-satis.js?v=2"></script>
<script src="assets/barkod-satis-gecmis.js?v=16"></script>
