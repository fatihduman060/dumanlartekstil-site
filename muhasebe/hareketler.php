...
if ($q !== '') { $where[]='(m.description LIKE ? OR c.name LIKE ? OR cat.name LIKE ? OR a.name LIKE ? OR m.document_name LIKE ?)'; array_push($params,"%$q%","%$q%","%$q%","%$q%","%$q%"); }
  ...
  <article class="panel-card form-card">
    ...
    <form method="post" enctype="multipart/form-data" class="stack-form">
      ...
      <div class="two-col">
        ...
        <label>Ödeme/Kasa hesabı<select name="account_id"><option value="">Kasa/banka seçilmedi</option><?php foreach($accounts as $a): ?><option value="<?php echo e($a['id']); ?>" <?php echo ((string)($edit['account_id'] ?? '')===(string)$a['id'])?'selected':''; ?>><?php echo e($a['name']); ?> — <?php echo e(account_type_label($a['account_type'])); ?></option><?php endforeach; ?></select><small>Tahsilat/gelir giriş, ödeme/gider çıkış olarak işler.</small></label>
      </div>
      <div class="two-col">
