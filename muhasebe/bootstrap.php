...
function init_db(PDO $pdo): void
{
    ...
    ensure_column($pdo, 'movements', 'check_id', 'INTEGER');
    ...
    // v50.22+: çek kayıtları ile cari hareketler tek kaynağa bağlanır.
    if ($currentSchemaVersion < 5022) {
        migrate_check_movement_links($pdo);
        repair_account_sync(false);
    }

    // v50.23+: Eski çekler, net girilen cari bakiyeleri bozmasın diye denge hareketi oluşturulur.
    if ($currentSchemaVersion < 5023) {
        migrate_opening_check_adjustments($pdo);
        repair_account_sync(false);
    }
...
function destroy_session_cookie(): void
{
    ...
    if (ini_get('session.use_cookies')) {
        ...
        setcookie('bitke_pwa_device', '', time() - 42000, $params['path'], $params['domain'], $params['secure'], true);
    }
    session_destroy();
}
function enforce_session_timeout(): void
{
    ...
    $timeout = !empty($_SESSION['remember_pwa']) ? PWA_SESSION_TIMEOUT_SECONDS : SESSION_TIMEOUT_SECONDS;
...
function remember_pwa_session(): void
{
    global $isHttps;
    $_SESSION['remember_pwa'] = true;
    $expires = time() + PWA_SESSION_COOKIE_SECONDS;
    setcookie('bitke_pwa_device', '1', $expires, APP_BASE_PATH, '', $isHttps, true);
    setcookie(session_name(), session_id(), $expires, APP_BASE_PATH, '', $isHttps, true);
}

function forget_pwa_session(): void
{
    global $isHttps;
    unset($_SESSION['remember_pwa']);
    setcookie('bitke_pwa_device', '', time() - 42000, APP_BASE_PATH, '', $isHttps, true);
    setcookie(session_name(), session_id(), 0, APP_BASE_PATH, '', $isHttps, true);
}
...
{
    // $pendingOnly geriye uyumluluk için duruyor; v50.22'de çeklerde manuel durum takibi ekrandan kaldırıldı.
...
{
    $stmt = db()->prepare("SELECT COUNT(*) FROM checks WHERE COALESCE(is_cancelled,0)=0 AND due_date < ?");
...
function normalize_check_no(?string $value): string
{
    ...
    return ltrim($value, 'Z');
}

function movement_is_check_like(array $m): bool
{
    $method = strtoupper((string)($m['payment_method'] ?? ''));
    $docType = (string)($m['document_type'] ?? '');
    $hasCheckMarker = (strpos($method, 'ÇEK') !== false) || (strpos($method, 'CEK') !== false) || $docType === 'cek_gorseli';
    return $hasCheckMarker && !empty($m['due_date']) && in_array((string)($m['movement_type'] ?? ''), ['tahsilat','odeme'], true);
}

function check_direction_from_movement_type(string $type): ?string
{
    if ($type === 'tahsilat') return 'alinacak';
    if ($type === 'odeme') return 'verilecek';
    return null;
}

function movement_type_from_check_direction(string $direction): string
{
    return $direction === 'verilecek' ? 'odeme' : 'tahsilat';
}

function check_category_id(PDO $pdo): ?int
{
    ...
    if ($id) return (int)$id;
    $stmt = $pdo->prepare('INSERT INTO categories (name, type, created_at) VALUES (?, ?, ?)');
    $stmt->execute(['Çek', 'genel', now()]);
    return (int)$pdo->lastInsertId();
}

function check_movement_description(array $ch): string
{
    $parts = ['Çek'];
    if (!empty($ch['bank_name'])) $parts[] = trim((string)$ch['bank_name']);
    if (!empty($ch['check_no'])) $parts[] = 'No: ' . trim((string)$ch['check_no']);
    if (!empty($ch['drawer'])) $parts[] = 'Keşideci: ' . trim((string)$ch['drawer']);
    if (!empty($ch['description'])) $parts[] = trim((string)$ch['description']);
    return implode(' / ', $parts);
}

function find_matching_check_for_movement(PDO $pdo, array $m): ?array
{
    $direction = check_direction_from_movement_type((string)$m['movement_type']);
    if (!$direction || empty($m['cari_id']) || empty($m['due_date'])) return null;
    $stmt = $pdo->prepare("SELECT * FROM checks WHERE COALESCE(is_cancelled,0)=0 AND direction=? AND cari_id=? AND due_date=? AND ABS(amount - ?) < 0.01 ORDER BY CASE WHEN movement_id IS NULL THEN 0 ELSE 1 END, id ASC LIMIT 1");
    $stmt->execute([$direction, (int)$m['cari_id'], $m['due_date'], (float)$m['amount']]);
    $row = $stmt->fetch();
    return $row ?: null;
}

function sync_check_to_movement(int $checkId, bool $allowCreate = true): void
{
    $pdo = db();
    ...
    $movementId = !empty($ch['movement_id']) ? (int)$ch['movement_id'] : 0;
    if ((int)($ch['is_cancelled'] ?? 0) === 1) {
        if ($movementId > 0) {
            $mStmt = $pdo->prepare('SELECT * FROM movements WHERE id=?');
            ...
            if ($m && (int)($m['is_cancelled'] ?? 0) === 0) {
                $pdo->prepare('UPDATE movements SET is_cancelled=1, cancelled_at=?, cancelled_by=?, cancel_reason=?, updated_at=? WHERE id=?')
                    ->execute([now(), current_user()['id'] ?? null, 'Bağlı çek iptal edildi', now(), $movementId]);
    ...
    $type = movement_type_from_check_direction((string)$ch['direction']);
    $categoryId = check_category_id($pdo);
    $movementDate = $ch['issue_date'] ?: substr((string)($ch['created_at'] ?? now()), 0, 10);
    $description = check_movement_description($ch);

    if ($movementId > 0) {
        $mStmt = $pdo->prepare('SELECT id FROM movements WHERE id=?');
        $mStmt->execute([$movementId]);
        if (!$mStmt->fetchColumn()) $movementId = 0;
    }

    if ($movementId <= 0 && $allowCreate) {
        $matchStmt = $pdo->prepare("SELECT id FROM movements WHERE COALESCE(is_cancelled,0)=0 AND check_id IS NULL AND cari_id=? AND movement_type=? AND due_date=? AND ABS(amount - ?) < 0.01 AND (UPPER(COALESCE(payment_method,'')) LIKE '%ÇEK%' OR UPPER(COALESCE(payment_method,'')) LIKE '%CEK%' OR document_type='cek_gorseli') ORDER BY id ASC LIMIT 1");
        $matchStmt->execute([(int)$ch['cari_id'], $type, $ch['due_date'], (float)$ch['amount']]);
        $movementId = (int)($matchStmt->fetchColumn() ?: 0);
    }

    if ($movementId > 0) {
        $pdo->prepare('UPDATE movements SET cari_id=?, category_id=?, account_id=NULL, movement_type=?, amount=?, movement_date=?, due_date=?, payment_method=?, description=?, document_type=?, document_path=?, document_name=?, document_mime=?, check_id=?, updated_at=?, is_cancelled=0, cancelled_at=NULL, cancelled_by=NULL, cancel_reason=NULL WHERE id=?')
            ->execute([(int)$ch['cari_id'] ?: null, $categoryId, $type, (float)$ch['amount'], $movementDate, $ch['due_date'], 'ÇEK', $description, 'cek_gorseli', $ch['document_path'] ?? null, $ch['document_name'] ?? null, $ch['document_mime'] ?? null, $checkId, now(), $movementId]);
    } elseif ($allowCreate) {
        $pdo->prepare('INSERT INTO movements (cari_id, category_id, account_id, movement_type, amount, movement_date, due_date, payment_method, description, document_type, document_path, document_name, document_mime, check_id, created_by, created_at, updated_at) VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
            ->execute([(int)$ch['cari_id'] ?: null, $categoryId, $type, (float)$ch['amount'], $movementDate, $ch['due_date'], 'ÇEK', $description, 'cek_gorseli', $ch['document_path'] ?? null, $ch['document_name'] ?? null, $ch['document_mime'] ?? null, $checkId, $ch['created_by'] ?: (current_user()['id'] ?? null), now(), now()]);
        $movementId = (int)$pdo->lastInsertId();
    }

    if ($movementId > 0) {
        $pdo->prepare('UPDATE checks SET movement_id=?, updated_at=? WHERE id=?')->execute([$movementId, now(), $checkId]);
        sync_movement_account_transaction($movementId);
    }
    sync_check_account_transaction($checkId);
    sync_check_balance_adjustment($checkId);
...
function sync_movement_to_check(int $movementId, bool $allowCreate = true): void
{
    ...
    if ((int)($m['is_cancelled'] ?? 0) === 1) {
        if ($checkId > 0) {
            ...
            if ($ch && (int)($ch['is_cancelled'] ?? 0) === 0) {
                $pdo->prepare('UPDATE checks SET is_cancelled=1, cancelled_at=?, cancelled_by=?, cancel_reason=?, updated_at=? WHERE id=?')
                    ->execute([now(), current_user()['id'] ?? null, 'Bağlı hareket iptal edildi', now(), $checkId]);
                sync_check_account_transaction($checkId);
            }
        }
        return;
    ...
    if (!movement_is_check_like($m)) return;
    $direction = check_direction_from_movement_type((string)$m['movement_type']);
    if (!$direction) return;
    $existing = $checkId > 0 ? null : find_matching_check_for_movement($pdo, $m);
    if ($existing) $checkId = (int)$existing['id'];

    $payload = [(int)$m['cari_id'] ?: null, $movementId, $direction, 'bekliyor', (float)$m['amount'], $m['movement_date'], $m['due_date'], null, null, null, null, trim((string)($m['description'] ?? '')), $m['document_path'] ?? null, $m['document_name'] ?? null, $m['document_mime'] ?? null];

    if ($checkId > 0) {
        $pdo->prepare("UPDATE checks SET cari_id=?, movement_id=?, direction=?, status=?, amount=?, issue_date=?, due_date=?, bank_name=COALESCE(bank_name, ?), branch_name=COALESCE(branch_name, ?), check_no=COALESCE(check_no, ?), drawer=COALESCE(drawer, ?), description=CASE WHEN COALESCE(description,'')='' THEN ? ELSE description END, document_path=COALESCE(document_path, ?), document_name=COALESCE(document_name, ?), document_mime=COALESCE(document_mime, ?), updated_at=?, is_cancelled=0, cancelled_at=NULL, cancelled_by=NULL, cancel_reason=NULL WHERE id=?")
            ->execute(array_merge($payload, [now(), $checkId]));
    } elseif ($allowCreate) {
        $pdo->prepare('INSERT INTO checks (cari_id, movement_id, direction, status, amount, issue_date, due_date, bank_name, branch_name, check_no, drawer, description, document_path, document_name, document_mime, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
            ->execute(array_merge($payload, [$m['created_by'] ?: (current_user()['id'] ?? null), now(), now()]));
        $checkId = (int)$pdo->lastInsertId();
    }

    if ($checkId > 0) {
        $pdo->prepare('UPDATE movements SET check_id=?, account_id=NULL, payment_method=?, document_type=?, updated_at=? WHERE id=?')
            ->execute([$checkId, 'ÇEK', 'cek_gorseli', now(), $movementId]);
        $pdo->prepare('UPDATE checks SET movement_id=?, updated_at=? WHERE id=?')->execute([$movementId, now(), $checkId]);
        sync_movement_account_transaction($movementId);
        sync_check_account_transaction($checkId);
        sync_check_balance_adjustment($checkId);
...
function balance_movement_type_from_check_direction(string $direction): string
{
    return $direction === 'verilecek' ? 'verecek' : 'alacak';
}

function check_adjustment_description(array $ch): string
{
    $parts = ['Eski çek devir dengeleme'];
    if (!empty($ch['bank_name'])) $parts[] = trim((string)$ch['bank_name']);
    if (!empty($ch['check_no'])) $parts[] = 'No: ' . trim((string)$ch['check_no']);
    if (!empty($ch['due_date'])) $parts[] = 'Vade: ' . tr_date($ch['due_date']);
    return implode(' / ', $parts);
}

function sync_check_balance_adjustment(int $checkId): void
{
    $pdo = db();
    $stmt = $pdo->prepare('SELECT * FROM checks WHERE id=?');
    $stmt->execute([$checkId]);
    ...
    $adjustmentId = !empty($ch['adjustment_movement_id']) ? (int)$ch['adjustment_movement_id'] : 0;
    $enabled = (int)($ch['is_opening_balance_check'] ?? 0) === 1 && (int)($ch['is_cancelled'] ?? 0) === 0 && !empty($ch['cari_id']);

    if (!$enabled) {
        if ($adjustmentId > 0) {
            $mStmt = $pdo->prepare('SELECT * FROM movements WHERE id=?');
            $mStmt->execute([$adjustmentId]);
            $m = $mStmt->fetch();
            if ($m && (int)($m['is_cancelled'] ?? 0) === 0) {
                $pdo->prepare('UPDATE movements SET is_cancelled=1, cancelled_at=?, cancelled_by=?, cancel_reason=?, updated_at=? WHERE id=?')
                    ->execute([now(), current_user()['id'] ?? null, 'Bağlı çek devir dengelemesi kapatıldı', now(), $adjustmentId]);
    ...
    $type = balance_movement_type_from_check_direction((string)$ch['direction']);
    $categoryId = check_category_id($pdo);
    $movementDate = $ch['issue_date'] ?: substr((string)($ch['created_at'] ?? now()), 0, 10);
    $description = check_adjustment_description($ch);

    if ($adjustmentId > 0) {
        $mStmt = $pdo->prepare('SELECT id FROM movements WHERE id=?');
        $mStmt->execute([$adjustmentId]);
        if (!$mStmt->fetchColumn()) $adjustmentId = 0;
    }

    if ($adjustmentId <= 0) {
        $matchStmt = $pdo->prepare("SELECT id FROM movements WHERE COALESCE(is_cancelled,0)=0 AND COALESCE(is_check_adjustment,0)=1 AND check_id=? ORDER BY id ASC LIMIT 1");
        $matchStmt->execute([$checkId]);
        $adjustmentId = (int)($matchStmt->fetchColumn() ?: 0);
    }

    if ($adjustmentId > 0) {
        $pdo->prepare('UPDATE movements SET cari_id=?, category_id=?, account_id=NULL, movement_type=?, amount=?, movement_date=?, due_date=?, payment_method=?, description=?, document_type=NULL, document_path=NULL, document_name=NULL, document_mime=NULL, check_id=?, is_check_adjustment=1, updated_at=?, is_cancelled=0, cancelled_at=NULL, cancelled_by=NULL, cancel_reason=NULL WHERE id=?')
            ->execute([(int)$ch['cari_id'], $categoryId, $type, (float)$ch['amount'], $movementDate, $ch['due_date'], 'ÇEK DEVİR', $description, $checkId, now(), $adjustmentId]);
    } else {
        $pdo->prepare('INSERT INTO movements (cari_id, category_id, account_id, movement_type, amount, movement_date, due_date, payment_method, description, document_type, document_path, document_name, document_mime, check_id, is_check_adjustment, created_by, created_at, updated_at) VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, NULL, NULL, NULL, NULL, ?, 1, ?, ?, ?)')
            ->execute([(int)$ch['cari_id'], $categoryId, $type, (float)$ch['amount'], $movementDate, $ch['due_date'], 'ÇEK DEVİR', $description, $checkId, $ch['created_by'] ?: (current_user()['id'] ?? null), now(), now()]);
...
function migrate_opening_check_adjustments(PDO $pdo): void
{
    ...
    try {
        $rows = $pdo->query("SELECT id FROM checks WHERE COALESCE(is_cancelled,0)=0 AND cari_id IS NOT NULL ORDER BY id ASC")->fetchAll(PDO::FETCH_COLUMN);
        $count = 0;
        foreach ($rows as $checkId) {
            $pdo->prepare('UPDATE checks SET is_opening_balance_check=1, updated_at=? WHERE id=?')
                ->execute([now(), (int)$checkId]);
            sync_check_to_movement((int)$checkId, true);
            sync_check_balance_adjustment((int)$checkId);
            $count++;
        }
        $pdo->prepare('INSERT INTO audit_logs (user_id, username, entity_type, entity_id, action, old_value, new_value, detail, ip, created_at) VALUES (NULL, ?, ?, NULL, ?, NULL, ?, ?, ?, ?)')
            ->execute(['sistem', 'cek_devir_denge', 'otomatik_duzeltme', json_encode(['schema'=>'5023','check_count'=>$count], JSON_UNESCAPED_UNICODE), 'Eski çekler cari bakiyeyi ikinci kez düşürmesin diye devir dengeleme hareketleri oluşturuldu.', $_SERVER['REMOTE_ADDR'] ?? '', now()]);
        $pdo->commit();
    } catch (Throwable $e) {
...
function merge_duplicate_checks(PDO $pdo): int
{
    $merged = 0;
    $groups = $pdo->query("SELECT cari_id, direction, due_date, ROUND(amount,2) AS amount_key, COUNT(*) AS cnt FROM checks WHERE COALESCE(is_cancelled,0)=0 GROUP BY cari_id, direction, due_date, ROUND(amount,2) HAVING cnt > 1")->fetchAll();
    foreach ($groups as $g) {
        $stmt = $pdo->prepare("SELECT * FROM checks WHERE COALESCE(is_cancelled,0)=0 AND cari_id IS ? AND direction=? AND due_date=? AND ROUND(amount,2)=? ORDER BY id ASC");
        $stmt->execute([$g['cari_id'], $g['direction'], $g['due_date'], $g['amount_key']]);
        $rows = $stmt->fetchAll();
        if (count($rows) < 2) continue;
        $norms = array_values(array_unique(array_filter(array_map(fn($r) => normalize_check_no($r['check_no'] ?? ''), $rows))));
        if (count($norms) > 1) continue;
        usort($rows, function($a, $b) { $ad = !empty($a['document_path']) ? 0 : 1; $bd = !empty($b['document_path']) ? 0 : 1; if ($ad !== $bd) return $ad <=> $bd; return ((int)$a['id']) <=> ((int)$b['id']); });
        $keep = array_shift($rows); $keepId = (int)$keep['id']; $updates = []; $params = [];
        foreach ($rows as $dup) {
            foreach (['bank_name','branch_name','check_no','drawer','description','document_path','document_name','document_mime','account_id'] as $col) {
                if (($keep[$col] ?? null) === null || (string)($keep[$col] ?? '') === '') {
                    if (($dup[$col] ?? null) !== null && (string)$dup[$col] !== '') { $updates[$col] = $dup[$col]; $keep[$col] = $dup[$col]; }
                }
            }
            if ((!empty($keep['issue_date']) && $keep['issue_date'] === $keep['due_date']) && !empty($dup['issue_date']) && $dup['issue_date'] !== $dup['due_date']) { $updates['issue_date'] = $dup['issue_date']; $keep['issue_date'] = $dup['issue_date']; }
            if (!empty($dup['movement_id']) && empty($keep['movement_id'])) { $updates['movement_id'] = $dup['movement_id']; $keep['movement_id'] = $dup['movement_id']; }
            if ((int)($dup['is_opening_balance_check'] ?? 0) === 1 && (int)($keep['is_opening_balance_check'] ?? 0) !== 1) { $updates['is_opening_balance_check'] = 1; $keep['is_opening_balance_check'] = 1; }
            $pdo->prepare('UPDATE movements SET check_id=?, updated_at=? WHERE check_id=?')->execute([$keepId, now(), (int)$dup['id']]);
            $pdo->prepare('UPDATE checks SET is_cancelled=1, cancelled_at=?, cancelled_by=NULL, cancel_reason=?, updated_at=? WHERE id=?')->execute([now(), 'Otomatik mükerrer çek birleştirildi (#' . $keepId . ')', now(), (int)$dup['id']]);
            sync_check_account_transaction((int)$dup['id']);
            sync_check_balance_adjustment((int)$dup['id']);
            $merged++;
        }
        if ($updates) { $set = []; foreach ($updates as $col => $val) { $set[] = $col . '=?'; $params[] = $val; } $set[] = 'updated_at=?'; $params[] = now(); $params[] = $keepId; $pdo->prepare('UPDATE checks SET ' . implode(', ', $set) . ' WHERE id=?')->execute($params); }
    }
    return $merged;
}

function migrate_check_movement_links(PDO $pdo): void
{
    $pdo->beginTransaction();
    try {
        merge_duplicate_checks($pdo);
        $movements = $pdo->query("SELECT * FROM movements WHERE COALESCE(is_cancelled,0)=0 AND check_id IS NULL ORDER BY id ASC")->fetchAll();
        foreach ($movements as $m) {
            if (!movement_is_check_like($m)) continue;
            $match = find_matching_check_for_movement($pdo, $m);
            if ($match) {
                $pdo->prepare('UPDATE movements SET check_id=?, account_id=NULL, payment_method=?, document_type=?, updated_at=? WHERE id=?')->execute([(int)$match['id'], 'ÇEK', 'cek_gorseli', now(), (int)$m['id']]);
                $pdo->prepare('UPDATE checks SET movement_id=?, updated_at=? WHERE id=?')->execute([(int)$m['id'], now(), (int)$match['id']]);
            } else {
                sync_movement_to_check((int)$m['id'], true);
            }
        }
        $checks = $pdo->query("SELECT id FROM checks WHERE COALESCE(is_cancelled,0)=0 ORDER BY id ASC")->fetchAll(PDO::FETCH_COLUMN);
        foreach ($checks as $checkId) sync_check_to_movement((int)$checkId, true);
        $pdo->prepare('INSERT INTO audit_logs (user_id, username, entity_type, entity_id, action, old_value, new_value, detail, ip, created_at) VALUES (NULL, ?, ?, NULL, ?, NULL, ?, ?, ?, ?)')
            ->execute(['sistem', 'cek_hareket', 'otomatik_baglanti', json_encode(['schema'=>'5022'], JSON_UNESCAPED_UNICODE), 'Çek ve hareket kayıtları otomatik bağlandı; mükerrer çekler pasifleştirildi.', $_SERVER['REMOTE_ADDR'] ?? '', now()]);
        $pdo->commit();
...
function repair_account_sync(bool $withLog = true): array
{
    ...
    try {
        ...
        // v50.22: Çeklerin kasa/banka etkisi manuel durumdan ayrıldı.
        // Çek cari hareket oluşturur; gerçek kasa/banka girişi/çıkışı ayrıca hareket olarak takip edilir.
