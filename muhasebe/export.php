...
if ($type === 'checks') {
    ...
    $stmt=db()->prepare($sql); $stmt->execute($params); $rows=$stmt->fetchAll();
    fputcsv($out,['ID','Yön','İptal Durumu','Çek Tarihi','Vade','Cari','Kasa/Banka','Banka','Şube','Çek No','Keşideci','Açıklama','Tutar','Belge'], ';');
    foreach($rows as $r) fputcsv($out,[$r['id'],check_direction_label($r['direction']),((int)($r['is_cancelled'] ?? 0)===1?'İptal':'Aktif'),$r['issue_date'],$r['due_date'],$r['cari_name'],$r['account_name'],$r['bank_name'],$r['branch_name'],$r['check_no'],$r['drawer'],$r['description'],number_format((float)$r['amount'],2,',','.'),$r['document_name']], ';');
