...
function banka_kart_adi(array $account): string
{
    $bank = trim((string)($account['bank_name'] ?? ''));
    if ($bank !== '') return $bank;
    return trim((string)($account['name'] ?? '')) ?: 'Banka adı girilmemiş';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_account') {
        ...
        if ($type === 'kasa') $bankName = '';
        $payload = [$type, $name, trim($_POST['iban'] ?? ''), $bankName, $opening, isset($_POST['is_active']) ? 1 : 0, trim($_POST['notes'] ?? '')];
        if ($id > 0) {
            ...
            log_action('Kasa/Banka hesabı güncellendi', $name); audit_action('hesap', $id, 'guncellendi', $oldAccount, ['type'=>$type,'name'=>$name,'bank_name'=>$bankName,'opening_balance'=>$opening,'is_active'=>isset($_POST['is_active']) ? 1 : 0], $name);
            ...
            log_action('Kasa/Banka hesabı eklendi', $name); audit_action('hesap', $newAccountId, 'eklendi', null, ['type'=>$type,'name'=>$name,'bank_name'=>$bankName,'opening_balance'=>$opening,'is_active'=>isset($_POST['is_active']) ? 1 : 0], $name);
...
$bankList = turkiye_banka_listesi();
$bankCards = [];
...
<section class="panel-card">
  ...
  <?php if (!$bankCards): ?>
    <p class="empty">Henüz banka hesabı tanımlı değil. Yeni hesap eklerken hesap tipini Banka seçip banka adını listeden seçebilirsin.</p>
  <?php else: ?>
    <section class="stats-grid four">
...
<section class="form-grid">
  <article class="panel-card form-card">
    ...
    <form method="post" class="stack-form">
      ...
      <div class="two-col">
        ...
        <label>Hesap adı<input name="name" required value="<?php echo e($edit['name'] ?? ''); ?>" placeholder="Garanti TL / Ana Kasa"></label>
      </div>
      <div class="two-col">
        <label>Banka adı<select name="bank_name">
          <option value="">Kasa / banka seçilmedi</option>
          <?php $currentBank = trim((string)($edit['bank_name'] ?? '')); if ($currentBank !== '' && !in_array($currentBank, $bankList, true)): ?><option value="<?php echo e($currentBank); ?>" selected><?php echo e($currentBank); ?></option><?php endif; ?>
          <?php foreach($bankList as $bankName): ?><option value="<?php echo e($bankName); ?>" <?php echo $currentBank===$bankName?'selected':''; ?>><?php echo e($bankName); ?></option><?php endforeach; ?>
        </select><small>Banka hesabı için listeden seç; kasa hesabında boş bırakabilirsin.</small></label>
      ...
      <label>IBAN<input name="iban" value="<?php echo e($edit['iban'] ?? ''); ?>" placeholder="TR__ ____ ____ ____ ____ ____ __"></label>
  ...
  <article class="panel-card">
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>Hesap</th><th>Tip</th><th>Banka / IBAN</th><th>Durum</th><th class="right">Bakiye</th><th></th></tr></thead>
        <tbody>
    ...
    <table>
      ...
      <?php foreach($transactions as $tr): ?>
        <tr>
          ...
          <td><strong><?php echo e($tr['account_name']); ?></strong><small><?php echo e($tr['bank_name'] ?: account_type_label($tr['account_type'])); ?></small></td>
