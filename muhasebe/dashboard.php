...
function dashboard_real_cash_filter_sql(): string
{
    // Çek ve çek devir hareketleri cari bakiyeyi etkiler; gerçek kasa/banka nakit akışına dahil edilmez.
    return "
          AND COALESCE(is_check_adjustment,0)=0
          AND (check_id IS NULL OR check_id=0)
          AND UPPER(COALESCE(payment_method,'')) NOT LIKE '%ÇEK%'
          AND UPPER(COALESCE(payment_method,'')) NOT LIKE '%CEK%'";
}

function dashboard_real_cash_totals(?string $start = null, ?string $end = null): array
{
    $where=[]; $params=[];
    if ($start) { $where[]='movement_date >= ?'; $params[]=$start; }
    if ($end) { $where[]='movement_date <= ?'; $params[]=$end; }
    $where[]='COALESCE(is_cancelled,0)=0';
    $where[]="movement_type IN ('tahsilat','gelir','odeme','gider')";
    $where[]='COALESCE(is_check_adjustment,0)=0';
    $where[]="(check_id IS NULL OR check_id=0)";
    $where[]="UPPER(COALESCE(payment_method,'')) NOT LIKE '%ÇEK%'";
    $where[]="UPPER(COALESCE(payment_method,'')) NOT LIKE '%CEK%'";
    $sql='SELECT movement_type, SUM(amount) AS total FROM movements WHERE ' . implode(' AND ', $where) . ' GROUP BY movement_type';
    $stmt = db()->prepare($sql); $stmt->execute($params);
    $totals=['tahsilat'=>0,'gelir'=>0,'odeme'=>0,'gider'=>0];
...
function dashboard_cashflow_rows(string $period): array
{
    ...
    $stmt = db()->prepare("SELECT {$sqlGroup} AS period_key, movement_type, SUM(amount) AS total
        ...
        WHERE movement_date >= ?
          ...
          " . dashboard_real_cash_filter_sql() . "
...
<section class="dashboard-section">
  ...
  <div class="stats-grid four section-stats">
    <article class="stat-card cash"><span>Bu ay giren para</span><strong class="text-success"><?php echo e(money($monthCashIn)); ?></strong><small>Gelir + tahsilat · çek hariç</small></article>
    <article class="stat-card cash"><span>Bu ay çıkan para</span><strong class="text-danger"><?php echo e(money($monthCashOut)); ?></strong><small>Gider + ödeme · çek hariç</small></article>
...
<section class="dashboard-section">
  <div class="dashboard-section-head">
    <div><span>Çek Takibi</span><h3>Çek vade takibi</h3></div>
    <p>Alınacak, verilecek, yaklaşan ve vadesi geçen çekler.</p>
  </div>
  <div class="stats-grid four section-stats">
    <article class="stat-card soft"><span>Alınacak çek</span><strong><?php echo e(money($checkTotals['alinacak'])); ?></strong><small><?php echo e($checkTotals['alinacak_count']); ?> adet</small></article>
    <article class="stat-card soft"><span>Verilecek çek</span><strong><?php echo e(money($checkTotals['verilecek'])); ?></strong><small><?php echo e($checkTotals['verilecek_count']); ?> adet</small></article>
...
<section class="dashboard-section smart-alert-section">
  ...
  <div class="content-grid dashboard-focus">
    ...
    <article class="panel-card">
      <div class="card-head"><h3>Çek vadesi yaklaşanlar</h3><a href="cekler.php?end=<?php echo e($weekAhead); ?>">Çekleri gör</a></div>
      <div class="cari-mini-list">
        <?php if (!$dueChecks): ?><p class="muted">Önümüzdeki 7 gün için çek vadesi yok.</p><?php endif; ?>
        <?php foreach(array_slice($dueChecks, 0, 6) as $ch): ?>
          <a class="mini-row alert-row" href="cekler.php">
...
<section id="nakit-akisi" class="content-grid chart-layout" style="scroll-margin-top:18px;">
  ...
  <article class="panel-card">
    ...
    <div class="info-list">
      ...
      <p><strong>Otomatik çıkış:</strong> <?php echo !empty($_SESSION['remember_pwa']) ? '30 gün' : ((int)(SESSION_TIMEOUT_SECONDS / 60) . ' dakika'); ?></p>
...
<section class="content-grid">
  <article class="panel-card">
    <div class="card-head"><h3>Vade uyarıları</h3><a href="cekler.php?end=<?php echo e($weekAhead); ?>">Çekleri gör</a></div>
    <div class="table-wrap">
      <table>
