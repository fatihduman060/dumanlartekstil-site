...
<script>
(function(){
  const btn = document.getElementById('shareWhatsappJpeg');
  const receiptNo = <?php echo json_encode((string)$receiptNo, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
  const customerName = <?php echo json_encode((string)$customer, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
  ...
  async function createJpegFile(){
    ...
    if (!page) throw new Error('Makbuz sayfası bulunamadı');
    const canvas = await html2canvas(page, {scale:2.4, backgroundColor:'#ffffff', useCORS:true, allowTaint:true, logging:false});
    const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/jpeg', 0.96));
    if (!blob) throw new Error('JPEG oluşturulamadı');
    const fileName = 'tahsilat-makbuzu-' + slug(receiptNo || customerName) + '.jpg';
    return {file:new File([blob], fileName, {type:'image/jpeg'}), blob, fileName};
  }
  async function shareJpeg(){
    const oldText = btn.textContent;
    btn.disabled = true;
    btn.textContent = 'JPEG hazırlanıyor...';
    try {
      const {file, blob, fileName} = await createJpegFile();
      ...
      alert('Makbuz JPEG formatına çevrilemedi. Lütfen tekrar deneyin.');
    } finally {
      btn.disabled = false;
      btn.textContent = oldText;
    }
  }
  btn?.addEventListener('click', shareJpeg);
})();
</script>
</body>
</html>
