(function(){
  ...
  function cellHtml(item){
    ...
    if(item.category){
      return '<button type="button" class="fatura-tur-button is-confirmed" data-fatura-tur-open="'+item.id+'"><strong>'+esc(item.category_label)+'</strong><small>Seçildi · değiştirmek için tıkla</small></button>';
    }
    if(item.suggestion){
      return '<button type="button" class="fatura-tur-button is-suggested" data-fatura-tur-open="'+item.id+'"><strong>'+esc(item.suggestion_label)+'</strong><small>Otomatik öneri · onayla/değiştir</small></button>';
    }
    return '<button type="button" class="fatura-tur-button is-empty" data-fatura-tur-open="'+item.id+'"><strong>Tür seç</strong><small>Telefon, iplik, doğalgaz...</small></button>';
  }
  ...
  function renderSummary(rows){
    ...
    summary.innerHTML='<div class="fatura-tur-summary-head"><strong>Aylık gelen fatura dağılımı</strong><small>TL faturalar · otomatik öneriler ayrıca belirtilir.</small></div>'+
      '<div class="fatura-tur-summary-grid">'+rows.map(function(row){
        ...
        return '<button type="button" data-fatura-tur-summary-filter="'+esc(row.key)+'"><span>'+esc(row.label)+'</span><strong>'+money(row.total)+'</strong><small>'+esc(note)+'</small></button>';
      }).join('')+'</div>';
  }
  ...
  function openModal(id){
    ...
    modal.querySelector('[data-fatura-tur-info]').innerHTML='<strong>'+esc(item.cari_name||'Cari seçilmedi')+'</strong><small>'+esc(item.document_name||'Fatura dosyası yok')+'</small>'+
  ...
  style.textContent=''
    ...
    +'.fatura-tur-summary{margin:12px 0 14px;padding:13px 14px;border:1px solid #d9e3dc;background:#f7fbf8;border-radius:15px;display:grid;gap:10px}.fatura-tur-summary-head{display:grid;gap:3px}.fatura-tur-summary-head strong{font-size:13px}.fatura-tur-summary-head small{font-size:10px;color:var(--muted)}.fatura-tur-summary-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:8px}.fatura-tur-summary-grid button{border:1px solid #d8dfda;background:#fff;border-radius:11px;padding:9px 10px;text-align:left;display:grid;gap:3px;cursor:pointer}.fatura-tur-summary-grid button:hover{border-color:#b99b65;background:#fffaf0}.fatura-tur-summary-grid span{font-size:9px;font-weight:850;color:#5c554b}.fatura-tur-summary-grid strong{font-size:13px}.fatura-tur-summary-grid small{font-size:8px;color:var(--muted)}'
