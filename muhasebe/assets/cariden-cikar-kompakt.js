...
(function () {
  ...
  var accountSelect = form.querySelector('[name="account_id"]');
  var csrf = form.querySelector('[name="csrf_token"]');
  if (!typeSelect || !categorySelect || !documentSelect || !cariSelect || !csrf) return;

  var defaultDrawer = 'Dumanlar Konfeksiyon Beyaz Eşya Ticaret Sanayi A.Ş.';
  ...
  style.textContent = [
    ...
    '.toplu-evrak-panel{width:min(1480px,100%);margin:auto;background:#fff;border-radius:20px;box-shadow:0 24px 70px rgba(0,0,0,.24);overflow:hidden}',
    ...
    '.toplu-evrak-columns,.toplu-evrak-row{display:grid;grid-template-columns:42px minmax(220px,1.7fr) minmax(180px,1.2fr) minmax(135px,.9fr) minmax(135px,.9fr) minmax(120px,.8fr) minmax(150px,1fr);gap:9px;align-items:center}',
    '.toplu-evrak-columns.senet,.toplu-evrak-row.senet{grid-template-columns:42px minmax(230px,2fr) minmax(150px,1fr) minmax(130px,1fr) minmax(160px,1fr)}',
    '.toplu-evrak-columns{padding:0 10px;color:#66706a;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.04em}',
    ...
    '.toplu-evrak-flow-note{margin:0 0 12px;padding:10px 12px;border-radius:12px;background:#fff8e7;color:#745000;font-size:12px;font-weight:800}',
    '@media(max-width:1050px){.toplu-evrak-columns{display:none}.toplu-evrak-row,.toplu-evrak-row.senet{grid-template-columns:36px 1fr 1fr}.toplu-evrak-row>strong{grid-row:1/5}.toplu-evrak-field label{display:block}}',
    '@media(max-width:820px){.toplu-evrak-backdrop{padding:10px}.toplu-evrak-row,.toplu-evrak-row.senet{grid-template-columns:36px 1fr}.toplu-evrak-row>strong{grid-row:1/8}.toplu-evrak-field{grid-column:2}.toplu-evrak-head{align-items:flex-start;flex-direction:column}}'
  ].join('');
  ...
  var subtitle = backdrop.querySelector('[data-subtitle]');
  var flowNote = backdrop.querySelector('[data-flow-note]');
  var columns = backdrop.querySelector('[data-columns]');
  var bankList = backdrop.querySelector('#topluEvrakBankalari');
  ...
  function bankOptions() {
    var names = [];
    if (accountSelect) {
      [].forEach.call(accountSelect.options, function (option) {
        if (!option.value) return;
        var text = String(option.textContent || '').replace(/\s+/g, ' ').trim();
        var low = text.toLocaleLowerCase('tr-TR');
        if (!text || low.indexOf('kasa') !== -1 || low.indexOf('nakit') !== -1) return;
        if (names.indexOf(text) === -1) names.push(text);
      });
    }
    ['Garanti BBVA','Türkiye İş Bankası','Ziraat Bankası','VakıfBank','Halkbank','Akbank','QNB','Yapı Kredi','DenizBank'].forEach(function (name) {
  ...
  function renderBankList() {
    bankList.innerHTML = '';
    bankOptions().forEach(function (name) {
      var option = document.createElement('option');
      option.value = name;
  ...
  function updateHeadings() {
    var isCheck = selectedInstrument() === 'cek';
    title.textContent = isCheck ? 'Toplu çek girişi' : 'Toplu senet girişi';
    if (isCheck && typeSelect.value === 'odeme') {
      subtitle.textContent = 'Verdiğimiz çeklerde keşideci Dumanlar olarak hazır gelir; banka ve çek numarası zorunludur.';
      flowNote.textContent = 'Ödeme: Keşideci otomatik doldurulur. Başka firmaya ait çek verilecekse adı değiştirebilirsin.';
    } else if (isCheck) {
      subtitle.textContent = 'Aldığımız çeklerde gerçek keşideciyi, bankayı ve çek numarasını yaz.';
      flowNote.textContent = 'Tahsilat: Çek müşterinin müşterisine ait olabilir. Bu yüzden keşideci adı manuel bırakılmıştır.';
    } else {
      subtitle.textContent = 'Her senedi ayrı satırda açıkça kontrol ederek kaydet.';
      flowNote.textContent = 'Senet girişinde banka ve çek numarası kullanılmaz.';
    }
    columns.className = 'toplu-evrak-columns' + (isCheck ? '' : ' senet');
    columns.innerHTML = isCheck
      ? '<span>No</span><span>Kişi / keşideci adı</span><span>Banka</span><span>Çek numarası</span><span>Tutar</span><span>İl</span><span>Vade tarihi</span>'
      : '<span>No</span><span>Kişi / keşideci adı</span><span>Tutar</span><span>İl</span><span>Vade tarihi</span>';
  }
  ...
  function renderRows() {
    ...
    rows.querySelectorAll('.toplu-evrak-row').forEach(function (row) {
      old.push({
        ...
        bank:(row.querySelector('[name="item_bank[]"]') || {}).value || '',
        checkNo:(row.querySelector('[name="item_check_no[]"]') || {}).value || '',
    ...
    var isCheck = selectedInstrument() === 'cek';
    var isPayment = typeSelect.value === 'odeme';
    countInput.value = count;
    rows.innerHTML = '';
    for (var i = 0; i < count; i++) {
      var saved = old[i] || {};
      var row = document.createElement('div');
      row.className = 'toplu-evrak-row' + (isCheck ? '' : ' senet');
      var personValue = saved.person || (isCheck && isPayment ? defaultDrawer : '');
      row.innerHTML = '<strong>' + (i + 1) + '</strong>' +
        ...
        (isCheck ? '<span class="toplu-evrak-field"><label>Banka</label><input name="item_bank[]" list="topluEvrakBankalari" placeholder="Banka seç veya yaz" required></span>' : '') +
        (isCheck ? '<span class="toplu-evrak-field"><label>Çek numarası</label><input name="item_check_no[]" inputmode="numeric" autocomplete="off" placeholder="Çek no" required></span>' : '') +
      ...
      row.querySelector('[name="item_person[]"]').value = personValue;
      if (isCheck) {
        row.querySelector('[name="item_bank[]"]').value = saved.bank || '';
        row.querySelector('[name="item_check_no[]"]').value = saved.checkNo || '';
      }
  ...
  function syncButton() {
    ...
    if (backdrop.classList.contains('open')) renderRows();
  }

  [typeSelect, categorySelect, documentSelect].forEach(function (el) { el.addEventListener('change', syncButton); });
  if (accountSelect) accountSelect.addEventListener('change', renderBankList);
