(function(){
  function formatAmount(value, currency){ var number=Number(value||0); try{return new Intl.NumberFormat('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2}).format(number)+' '+currency;}catch(e){return number.toFixed(2).replace('.',',')+' '+currency;} }
  function updateCell(cell, balances){ if(!cell||!balances||!balances.length)return;var visible=balances.filter(function(item){return Math.abs(Number(item.net||0))>=0.005||item.currency==='TL';});if(!visible.length)visible=[{currency:'TL',net:0}];cell.innerHTML=visible.map(function(item){var net=Number(item.net||0),cls=net>=0?'text-success':'text-danger',note=net>=0?'Alacaklı':'Biz borçluyuz';return '<strong class="'+cls+'" style="display:block;line-height:1.15">'+formatAmount(net,item.currency||'TL')+'</strong><small style="display:block;margin-bottom:3px">'+note+'</small>';}).join(''); }
  function parseMoney(text){var value=String(text||'').replace(/\s/g,'').replace(/TL/gi,'').replace(/\./g,'').replace(',','.').replace(/[^0-9.-]/g,'');var number=parseFloat(value);return Number.isFinite(number)?number:0;}
  ...
  function clearRestoredNewOffer(){
    ...
    var cari=form.querySelector('[name="cari_id"]');if(cari)cari.value='';
    ['customer_name','customer_city','customer_phone','customer_tax_office','customer_tax_no','customer_address','note','term_text'].forEach(function(name){var el=form.querySelector('[name="'+name+'"]');if(el)el.value='';});
    ...
    var discount=form.querySelector('[name="discount_enabled"]'),vat=form.querySelector('[name="vat_enabled"]'),discountRate=form.querySelector('[name="discount_rate"]'),vatRate=form.querySelector('[name="vat_rate"]');
