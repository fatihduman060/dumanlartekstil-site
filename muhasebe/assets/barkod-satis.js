(function(){
  ...
  function persistedItem(item){
    ...
    return {
      ...
      quantity:quantity,
      single_decrement_used:!!item.single_decrement_used
    };
  }
  ...
  function add(p){invalidateSaleRequest();var old=cart.find(function(x){return Number(x.id)===Number(p.id);});if(old){old.quantity+=1;old.single_decrement_used=false;}else{p.quantity=1;p.single_decrement_used=false;cart.push(p);}lastAddedProductId=Number(p.id);render();scan.value='';lastLookup='';scan.focus({preventScroll:true});status.textContent=productName(p)+' sepete eklendi.';revealLastAdded();}
  ...
  function lookup(){var q=scan.value.trim();if(/^\+/.test(q)){var shortcut=q.match(/^\+(\d+)$/);if(shortcut)addToLastProduct(Number(shortcut[1]));return;}if(!q||lookupBusy||q===lastLookup)return;var revision=++lookupRevision;lookupBusy=true;lastLookup=q;status.textContent='Barkod aranıyor…';window.posReadJson(api+'?action=barcode&barcode='+encodeURIComponent(q)+'&_='+Date.now()).then(function(d){if(revision!==lookupRevision||scan.value.trim()!==q)return null;if(d.product){add(d.product);results.hidden=true;return null;}return window.posReadJson(api+'?action=products&q='+encodeURIComponent(q)+'&_='+Date.now()).then(function(x){if(revision!==lookupRevision||scan.value.trim()!==q)return;showResults(x.products||[]);status.textContent='';});}).catch(function(error){if(revision!==lookupRevision||scan.value.trim()!==q)return;status.textContent=error.message||'Ürün aranamadı.';lastLookup='';}).finally(function(){lookupBusy=false;});}
  ...
  function cartEventPayload(eventType,items,reason,eventSource){
    var body=new FormData();body.set('action','cart_event');body.set('csrf_token',csrf);body.set('event_type',eventType);if(reason)body.set('removal_reason',reason);if(eventSource)body.set('event_source',eventSource);if(root.dataset.posTerminalId)body.set('terminal_id',root.dataset.posTerminalId);body.set('items_json',JSON.stringify(items.map(function(x){return {product_id:x.id,quantity:x.quantity};})));body.set('discount_amount',discount.value||0);var payment=root.querySelector('input[name="pos_payment"]:checked');body.set('payment_method',payment?payment.value:'cash');return body;
  }
  function logCartEvent(eventType,items,reason,eventSource){
    return fetch(api,{method:'POST',body:cartEventPayload(eventType,items,reason,eventSource),credentials:'same-origin',cache:'no-store'}).then(function(r){return r.json();}).then(function(d){if(!d.ok)throw new Error(d.error||'Sepet işlemi kaydedilemedi.');return d;});
  }
  function decreaseOneWithoutReason(item){
    ...
    item.single_decrement_pending=true;
    var removedItem=Object.assign({},item,{quantity:1});
    status.textContent='1 adet azaltılıyor…';
    logCartEvent('item_removed',[removedItem],'','single_decrement').then(function(d){
      var currentIndex=cart.indexOf(item);
      if(currentIndex<0)return;
      invalidateSaleRequest();
      cart[currentIndex].quantity=Math.max(1,Number(cart[currentIndex].quantity)-1);
      cart[currentIndex].single_decrement_used=true;
      cart[currentIndex].single_decrement_pending=false;
      render();
      status.textContent=d.message;
    }).catch(function(error){
      item.single_decrement_pending=false;
      status.textContent='Adet azaltılmadı: '+error.message;
    });
  }
  ...
  removalConfirm.onclick=function(){
    ...
    if(removalMode==='cart'){
      ...
      logCartEvent('cart_cleared',snapshot,reason,'cart_clear').then(function(d){
    ...
    if(removalMode==='quantity'){
      ...
      logCartEvent('item_removed',[removedItem],reason,'quantity_decrease').then(function(d){
        var currentIndex=cart.indexOf(item);if(currentIndex>=0){invalidateSaleRequest();cart[currentIndex].quantity=target;cart[currentIndex].single_decrement_used=true;cart[currentIndex].single_decrement_pending=false;}render();status.textContent=d.message;closeRemoval();
      }).catch(function(error){
        removalStatus.textContent='Adet azaltılmadı: '+error.message;removalConfirm.disabled=false;
      });
      return;
    }
    logCartEvent('item_removed',[item],reason,'item_remove').then(function(d){
  ...
  cartBox.addEventListener('click',function(e){
    ...
    if(b.hasAttribute('data-minus')){
      if(Number(cart[i].quantity)>1){decreaseOneWithoutReason(cart[i]);return;}
      return;
    }
    if(b.hasAttribute('data-plus')){invalidateSaleRequest();cart[i].quantity+=1;cart[i].single_decrement_used=false;}
  ...
  cartBox.addEventListener('change',function(e){
    ...
    if(newQuantity<oldQuantity){
      var removedQuantity=Math.round((oldQuantity-newQuantity)*1000)/1000;
      e.target.value=oldQuantity;
      if(Math.abs(removedQuantity-1)<0.001){decreaseOneWithoutReason(cart[i]);return;}
      openRemoval('quantity',cart[i],newQuantity);return;
    }
    if(newQuantity!==oldQuantity){invalidateSaleRequest();cart[i].quantity=newQuantity;cart[i].single_decrement_used=false;render();}
  });
  ...
  function closePaymentModal(){paymentModal.hidden=true;delete root.dataset.posSkipPrint;}
