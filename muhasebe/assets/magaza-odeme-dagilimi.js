(function(){
  ...
  var state={period:'',csrf:'',canWrite:false,canQuickCashLeft:false,previousRequest:0,cardDelayDays:13};
  ...
  function settlementDate(value){
    ...
    date.setDate(date.getDate()+state.cardDelayDays);
  ...
  function updatePreview(){
    ...
    preview.innerHTML=''
      +'<span><small>Mağaza Kasa · aynı gün</small><strong>'+money(amounts.cashTotal)+'</strong></span>'
      ...
      +'<em>Nakit aynı gün Mağaza Kasa’ya girer. Kart/POS tutarı satış günü 1. gün kabul edilerek 14. gün (satış tarihi +13 gün) Garanti Dumanlar’a işlenir. Veresiye satış ve bozuk para hesaplara aktarılmaz.</em>';
  }
  ...
  function buildPanel(){
    ...
    panel.innerHTML=''
      +'<div class="magaza-odeme-head"><div><strong>Günlük Satış Dağılımı</strong><small>Nakit girişler aynı gün Mağaza Kasa’ya; kart/POS girişleri satış günü 1. gün kabul edilerek 14. gün (satış tarihi +13 gün) Garanti Dumanlar hesabına otomatik işlenir. Veresiye satış ve kasada bırakılan bozuk para aktarılmaz.</small></div><span data-magaza-odeme-status></span></div>'
  ...
  function render(data){
    state.csrf=String(data.csrf_token||state.csrf);state.canWrite=!!data.can_write;state.canQuickCashLeft=!!data.can_quick_edit_cash_left;state.cardDelayDays=Math.max(0,Number(data.card_delay_days||13)||13);window.BITKE_FATIH_CASH_LEFT_EDIT=state.canQuickCashLeft;
    ...
    if(!items.length) list.innerHTML='<p class="muted">Bu dönemde henüz günlük satış dağılımı kaydı yok.</p>';
      ...
      +items.map(function(item){
        var cashStatus=item.cash_total_amount>0?(item.cash_posted?'Mağaza Kasa’ya işlendi':'İşlem gününü bekliyor'):'Nakit giriş yok';
    ...
    setStatus((summary.count||0)+' gün · nakit Mağaza Kasa’ya aynı gün · kart Garanti’ye 14. gün (+13 gün)','success');updatePreview();var activeDate=form?qs('[name="sale_date"]',form):null;if(activeDate) loadPreviousCashChange(activeDate.value);
  ...
  function deleteEntry(id){if(!id) return;var reason=window.prompt('Günlük satış kaydı iptal nedeni (en az 6 karakter):','');if(!reason||reason.trim().length<6){window.alert('İptal nedeni en az 6 karakter olmalıdır.');return;}if(!window.confirm('Kayıt arşive alınacak; bağlı otomatik kasa/banka hareketleri silinmeden iptal edilecek. Devam edilsin mi?')) return;var body=new FormData();body.set('action','delete');body.set('id',String(id));body.set('cancel_reason',reason.trim());body.set('period',state.period);body.set('csrf_token',state.csrf);fetch('magaza-odeme-dagilimi.php',{method:'POST',body:body,credentials:'same-origin',cache:'no-store'}).then(function(response){return response.json();}).then(function(data){if(!data.ok) throw new Error(data.error||'Kayıt iptal edilemedi.');render(data);setStatus('Günlük satış kaydı arşive alındı; bağlı otomatik hareketler iptal edilerek geçmişte korundu.','success');}).catch(function(error){setStatus(error.message||'Kayıt iptal edilemedi.','danger');});}
  ...
  var style=document.createElement('style');style.textContent=''
    ...
    +'@media(max-width:1350px){.magaza-odeme-summary{grid-template-columns:repeat(3,minmax(0,1fr))}.magaza-odeme-form{grid-template-columns:repeat(3,minmax(0,1fr))}.magaza-odeme-preview{grid-column:1/-1}}@media(max-width:650px){.magaza-odeme-head{display:grid}.magaza-odeme-summary,.magaza-odeme-form,.magaza-odeme-preview{grid-template-columns:1fr}.magaza-odeme-preview em{grid-column:1}.magaza-odeme-form .btn{width:100%}.magaza-onceki-bozuk{grid-template-columns:1fr}.magaza-onceki-bozuk small{grid-column:1}.magaza-odeme-list{display:none!important}}';document.head.appendChild(style);
