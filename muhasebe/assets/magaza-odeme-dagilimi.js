(function(){
  ...
  function formTotal(form){
    return numberValue(qs('[name="cash_amount"]',form).value)
      +numberValue(qs('[name="card_amount"]',form).value)
      +numberValue(qs('[name="credit_amount"]',form).value);
  }
  function updatePreview(){
    var form=qs('[data-magaza-odeme-form]');
    var preview=qs('[data-magaza-odeme-preview]');
    if(!form||!preview) return;
    preview.innerHTML='<strong>Günlük toplam: '+money(formTotal(form))+'</strong><small>Veresiye tahsilatı geçmiş satış tahsilatıdır; günlük satış toplamına tekrar eklenmez.</small>';
  }
  function buildPanel(){
    ...
    panel.innerHTML=''
      +'<div class="magaza-odeme-head"><div><strong>Günlük Satış Dağılımı</strong><small>Nakit, kredi kartı, veresiye ve veresiye tahsilatını günlük olarak ayrı kaydet. Bu tablo KDV ve fatura hesaplarından bağımsızdır.</small></div><span data-magaza-odeme-status></span></div>'
      +'<div class="magaza-odeme-summary">'
      +'<article><span>Nakit</span><strong data-magaza-odeme-cash>0,00 TL</strong></article>'
      +'<article><span>Kredi kartı</span><strong data-magaza-odeme-card>0,00 TL</strong></article>'
      +'<article><span>Veresiye</span><strong data-magaza-odeme-credit>0,00 TL</strong></article>'
      +'<article><span>Veresiye tahsilat</span><strong data-magaza-odeme-collection>0,00 TL</strong></article>'
      +'<article><span>Günlük toplamlar</span><strong data-magaza-odeme-total>0,00 TL</strong></article>'
      +'</div>'
      ...
      +'<label>Nakit<input type="text" inputmode="decimal" name="cash_amount" placeholder="0,00"></label>'
      +'<label>Kredi kartı<input type="text" inputmode="decimal" name="card_amount" placeholder="0,00"></label>'
      +'<label>Veresiye<input type="text" inputmode="decimal" name="credit_amount" placeholder="0,00"></label>'
      +'<label>Veresiye tahsilat<input type="text" inputmode="decimal" name="credit_collection_amount" placeholder="0,00"></label>'
  ...
  function render(data){
    ...
    qs('[data-magaza-odeme-collection]',panel).textContent=money(summary.credit_collection);
    ...
    if(!items.length){
      ...
      list.innerHTML='<div class="table-wrap"><table><thead><tr><th>Tarih</th><th>Nakit</th><th>Kredi kartı</th><th>Veresiye</th><th>Veresiye tahsilat</th><th>Günlük toplam</th><th></th></tr></thead><tbody>'
        +items.map(function(item){
          return '<tr data-magaza-odeme-row data-id="'+item.id+'" data-date="'+esc(item.sale_date)+'" data-cash="'+Number(item.cash_amount||0)+'" data-card="'+Number(item.card_amount||0)+'" data-credit="'+Number(item.credit_amount||0)+'" data-collection="'+Number(item.credit_collection_amount||0)+'">'
            +'<td><strong>'+esc(item.sale_date.split('-').reverse().join('.'))+'</strong></td>'
            +'<td>'+money(item.cash_amount)+'</td>'
            +'<td>'+money(item.card_amount)+'</td>'
            +'<td>'+money(item.credit_amount)+'</td>'
            +'<td>'+money(item.credit_collection_amount)+'</td>'
            ...
            +'</tr>';
        }).join('')+'</tbody><tfoot><tr class="magaza-odeme-total-row"><td><strong>GENEL TOPLAM</strong></td><td><strong>'+money(summary.cash)+'</strong></td><td><strong>'+money(summary.card)+'</strong></td><td><strong>'+money(summary.credit)+'</strong></td><td><strong>'+money(summary.credit_collection)+'</strong></td><td><strong>'+money(summary.daily_total)+'</strong></td><td></td></tr></tfoot></table></div>';
    }
    setStatus((summary.count||0)+' gün · '+money(summary.daily_total)+' satış toplamı','success');
    updatePreview();
  }
  function editEntry(button){
    ...
    qs('[name="credit_collection_amount"]',form).value=Number(row.getAttribute('data-collection')||0).toLocaleString('tr-TR',{minimumFractionDigits:2,maximumFractionDigits:2});
  ...
  function saveEntry(event){
    ...
    fetch('magaza-odeme-dagilimi.php',{method:'POST',body:body,credentials:'same-origin',cache:'no-store'})
      ...
      .then(function(data){
        ...
        qs('[name="sale_date"]',form).value=defaultDate(state.period);
        render(data);
        setStatus('Günlük satış dağılımı kaydedildi. KDV ve fatura hesaplarında değişiklik yapılmadı.','success');
      })
  ...
  style.textContent=''
    ...
    +'.magaza-odeme-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.magaza-odeme-head>div{display:grid;gap:4px}.magaza-odeme-head strong{font-size:15px}.magaza-odeme-head small{font-size:11px;color:var(--muted);max-width:760px}.magaza-odeme-head>span{font-size:10px;font-weight:850;color:var(--muted)}.magaza-odeme-head>span.is-success{color:#1f6b3d}.magaza-odeme-head>span.is-danger{color:#96352f}'
    +'.magaza-odeme-summary{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px}.magaza-odeme-summary article{display:grid;gap:5px;padding:10px 11px;border:1px solid #dce5df;border-radius:12px;background:#fff}.magaza-odeme-summary span{font-size:9px;color:var(--muted);font-weight:800}.magaza-odeme-summary strong{font-size:13px}'
    +'.magaza-odeme-form{display:grid;grid-template-columns:150px repeat(4,minmax(120px,1fr));gap:9px;align-items:end;padding:12px;border:1px solid #dce5df;border-radius:13px;background:#fff}.magaza-odeme-form label{display:grid;gap:5px;font-size:10px;font-weight:850}.magaza-odeme-form input{width:100%}.magaza-odeme-preview{grid-column:1/-2;display:flex;justify-content:space-between;gap:10px;align-items:center;padding:9px 11px;border-radius:10px;background:#f1f6f3}.magaza-odeme-preview strong{font-size:12px}.magaza-odeme-preview small{font-size:9px;color:var(--muted)}'
    +'.magaza-odeme-list .table-wrap{margin:0}.magaza-odeme-list table{width:100%;min-width:900px}.magaza-odeme-list th,.magaza-odeme-list td{font-size:10px}.magaza-odeme-actions{white-space:nowrap}.magaza-odeme-actions button{border:0;background:transparent;padding:3px 5px;font-size:9px;font-weight:800;color:#745f3e;text-decoration:underline;cursor:pointer}.magaza-odeme-total-row td{background:#f3f7f4;border-top:2px solid #cad8ce}'
    +'@media(max-width:1150px){.magaza-odeme-summary{grid-template-columns:repeat(3,minmax(0,1fr))}.magaza-odeme-form{grid-template-columns:repeat(2,minmax(0,1fr))}.magaza-odeme-preview{grid-column:1/-1}}'
    +'@media(max-width:650px){.magaza-odeme-head{display:grid}.magaza-odeme-summary,.magaza-odeme-form{grid-template-columns:1fr}.magaza-odeme-preview{display:grid}.magaza-odeme-form .btn{width:100%}}';
