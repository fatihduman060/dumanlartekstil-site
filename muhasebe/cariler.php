...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        if ($id > 0) {
            ...
            log_action('Cari güncellendi', $name); flash('success', 'Cari güncellendi.');
        } else {
            ...
            log_action('Cari eklendi', $name); flash('success', 'Cari eklendi.');
        }
        redirect('cariler.php');
    }
    if ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        $stmt = db()->prepare('SELECT name FROM cariler WHERE id=?'); $stmt->execute([$id]); $name = $stmt->fetchColumn();
        $stmt = db()->prepare('DELETE FROM cariler WHERE id=?'); $stmt->execute([$id]);
        log_action('Cari silindi', (string)$name); flash('success', 'Cari silindi. Bağlı hareket/çek varsa cari bağı kaldırıldı, kayıtlar korunur.'); redirect('cariler.php');
    }
}
...
$type = trim($_GET['type'] ?? '');
$where=[]; $params=[];
if ($q !== '') { $where[]='(name LIKE ? OR tax_no LIKE ? OR tax_office LIKE ? OR phone LIKE ? OR email LIKE ? OR authorized_person LIKE ? OR city LIKE ?)'; array_push($params, "%$q%","%$q%","%$q%","%$q%","%$q%","%$q%","%$q%"); }
