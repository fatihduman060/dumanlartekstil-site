...
function fatura_masraf_fisi_ekleyebilir(): bool
{
    return can_write() || is_murat_limited_user();
}

function fatura_masraf_fisi_payload(string $period): array
{
    ...
    return [
        ...
        'items' => $rows,
        // Silme gibi yönetici işlemleri mevcut yazma yetkisinde kalır.
        'can_write' => can_write(),
        'can_add' => fatura_masraf_fisi_ekleyebilir(),
...
try {
    if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
        ...
        if ($action === 'delete') {
            if (!can_write()) {
                throw new RuntimeException('Bu masraf fişini silmek için yönetici düzenleme yetkisi gerekir.');
            }
            ...
            if (!fatura_masraf_fisi_ekleyebilir()) {
                throw new RuntimeException('Masraf fişi ekleme yetkiniz yok.');
            }
    ...
    $payload = fatura_masraf_fisi_payload($period);
    $payload['ok'] = true;
